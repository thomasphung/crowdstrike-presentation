...
If you are not familiar with this space (like me), then you probably haven't heard about CrowdStrike until last month when 8.5 million Windows machines worldwide experienced the blue screen of death, causing system outages and service disruption.
:::
...
![Figure 1: CrowdStrike incident, showing interaction with CrowdStrike and remote Windows machines that use CrowdStrike's products](assets/images/crowdstrike-incident-diagram.png)
...
::: {.notes}
(1 minute)
But CrowdStrike's tests failed to catch this insecure code and inconsistent data. Their config file validation code was using RegEx! They used a wildcard pattern that accepts any number of inputs which is why when the updated the number of inputs in the config files, the tests still passed. But in their production environment, they were using an older version of their config file with 20 inputs when their code expects 21 inputs.

In addition, their developers also didn't do proper integration testing. If they deployed the updated config file in a local environment, mimicking what a customer experiences when they recieve the new update, they could have caught the blue screen of death before 8.5 million machines went down.
:::
...
::: {.notes}
(20 seconds)
So I'm over here scratching my head and asking why is such a large and pervasive company like CrowdStrike a single point of failure for a lot of organizations? In the realm of security and cybersecurity, you are not too big to fail. Everyone is vulnerable, but you can reduce risk of incidents through prevention steps and recover assets through remediation practices.
:::
